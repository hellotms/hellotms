    ...
    <div className="pt-16">
      ...
      <section className="relative py-20 sm:py-28 hero-gradient overflow-hidden">
        ...
        <div className="container relative z-10 text-center">
          <p className="text-[var(--accent)] text-xs font-bold tracking-widest uppercase mb-3">Our Work</p>
            ...
            {categories.map(cat => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${activeCategory === cat
                  ? 'bg-[var(--accent-dark)] text-white shadow-lg shadow-[var(--accent)]/30'
                  : 'bg-[var(--card)] text-[var(--muted)] border border-[var(--border)] hover:border-[var(--accent)]/50 hover:text-[var(--accent)]'
                  }`}
              >
                {cat}
              </button>
              ...
              {filteredProjects.map((project, i) => {
                ...
                return (
                  ...
                  >
                    <div className={`relative h-56 bg-gradient-to-br ${GRADIENT_CLASSES[gradientIdx]} overflow-hidden`}>
                      ...
                      {project.category && (
                        <div className="absolute top-3 left-3">
                          <span className="px-2.5 py-1 rounded-lg bg-[var(--accent-dark)] shadow-xl text-[10px] text-white font-bold uppercase tracking-wider">
                            {project.category}
                          </span>
                        </div>
                      )}
                    ...
                    <div className="p-5">
                      <h2 className="font-bold text-[var(--foreground)] group-hover:text-[var(--accent)] transition-colors line-clamp-1 mb-1">
                        {project.title}
                      </h2>
