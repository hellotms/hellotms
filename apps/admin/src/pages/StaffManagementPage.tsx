            ...
            <Modal isOpen={isInviteOpen} onClose={() => setIsInviteOpen(false)} title="System Enlistment" description="Issue a secure invitation to a new staff member.">
                <form onSubmit={form.handleSubmit(v => inviteMutation.mutate({ email: v.s_mail, name: v.s_name, role_id: v.s_role }))} className="space-y-6">
                    <div className="space-y-4">
                        <div><label className="text-[10px] font-black uppercase text-muted-foreground mb-1 block">Full Name <span className="text-red-500">*</span></label><input {...form.register('s_name', { required: true })} className="w-full bg-background border border-border rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/10 outline-none" /></div>
                        <div><label className="text-[10px] font-black uppercase text-muted-foreground mb-1 block">Staff-ID (Email) <span className="text-red-500">*</span></label><input type="email" {...form.register('s_mail', { required: true })} className="w-full bg-background border border-border rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/10 outline-none" /></div>
                        <div><label className="text-[10px] font-black uppercase text-muted-foreground mb-1 block">Strategic Role <span className="text-red-500">*</span></label><select {...form.register('s_role', { required: true })} className="w-full bg-background border border-border rounded-xl px-4 py-3 text-sm outline-none cursor-pointer">{roles.map(r => <option key={r.id} value={r.id}>{r.label}</option>)}</select></div>
                    </div>
